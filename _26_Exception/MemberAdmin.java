...
//		test();		// 예외처리를 해줘야하는 경우와 직접처리
		test1();	// 예외처리를 안해 줘도 되는데 해 본 코드
		test2();	// 예외처리를 헤ㅐ 줘야하는 경우이고 예외 떠 넘기기
	}
...
//			System.out.println(mm[1].getId());
	...
	private void test() { // 이 예제는 예외처리를 반드시 해 줘야 한다.
						  // 예외처리 방법은 직접처리 > try catch finally
		try {
			// try 블록은 예외가 발생할 코드를 작성한다.
			// 예외가 발생하면 더 이상 코드를 실행하지 않고 
			// catch 구문(블록)으로 이동
			File file = new File("Output.txt");
			FileWriter writer;
			writer = new FileWriter(file);
			...
			// catch 구문은 소괄호안의 예외가 발생했을 떄 처리해주는 블록
			e.printStackTrace();
		} finally {
			// try 구문을 빠져 나갈 때 실행된다.
			// 예외 발생 여부상관없이 실행된다.
		}
	}
}
